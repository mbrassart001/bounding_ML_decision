...
from exp.net_utils import train, matching_inputs, approx2robdd, compute_metrics
...
def main(filename):
    ...
    print(compute_metrics(model, x_valid, y_valid))
    ...
    if robdd_desc is not None:
        ...
        print(compute_metrics(GlobalModel(**new_modules), x_valid, y_valid))
        
        for label, robdd in new_modules.items():
            # retirer variables dans robdd
            if label == "up":
                robdd.consensus(remove_inputs)
            elif label == "down":
                robdd.smoothing(remove_inputs)
